...
void WebServer::AddRoute(const std::string& path, const std::string& method,
                         RequestHandler handler) {
  routes_[path] = {method, std::move(handler)};
  logger_->debug("Added route: {} {}", method, path);
}

void WebServer::AddRawRoute(const std::string& path, const std::string& method,
                            RawHandler handler) {
...
void WebServer::SetMountPoint(const std::string& mount,
...
void WebServer::server_loop() {
  ...
  if (!auth_token_.empty()) {
    http_server_->set_pre_routing_handler(
        [this](const httplib::Request& req,
          ...
          res.status = 401;
          res.set_content(create_error_response("Unauthorized", 401),
                          "application/json");
  ...
  for (const auto& [path, route_info] : routes_) {
    const auto& info =
        route_info;  // C++17: structured bindings cannot be captured in lambdas
    if (info.method == "GET") {
      http_server_->Get(path, [&info, this](const httplib::Request& req,
        ...
        try {
          ...
          res.status = 500;
          res.set_content(create_error_response("Internal server error", 500),
                          "application/json");
        }
      });
    } else if (info.method == "POST") {
      http_server_->Post(path, [&info, this](const httplib::Request& req,
                                             httplib::Response& res) {
        try {
